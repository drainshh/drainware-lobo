	...
	[ & ]( ) {
		...
		case HASH_BT( "player_hurt" ):
			...
			if ( GET_VARIABLE( g_variables.m_hit_sound, bool ) ) {
				static float last_hit_sound_time = 0.f;
				const float now = g_interfaces.m_global_vars_base ? g_interfaces.m_global_vars_base->m_real_time : 0.f;
				if (  m_attacker  == g_interfaces.m_engine_client->get_local_player( ) && now - last_hit_sound_time > 0.08f ) {
					g_interfaces.m_surface->play_sound( "ui\\beep07.wav" );
					last_hit_sound_time = now;
				}
			}
			...
			g_players.add_damage_indicator( m_victim, m_damage );
