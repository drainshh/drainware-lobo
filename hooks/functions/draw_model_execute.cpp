                                                          ...
                                                          matrix3x4_t* custom_bone_to_world )
{
	...
	const bool should_draw_attachment_vis = GET_VARIABLE( g_variables.m_player_visible_attachment_chams, bool );
	const bool should_draw_attachment_invis = GET_VARIABLE( g_variables.m_player_invisible_attachment_chams, bool );

	const bool should_draw_chams = should_draw_lag || should_draw_vis || should_draw_invis || should_draw_attachment_vis || should_draw_attachment_invis;
	...
	if ( !GET_VARIABLE( g_variables.m_ragdolls, bool ) && player && !player->is_alive( ) && mdl.find( "player" ) != std::string::npos )
		return;

	if ( mdl.find( "weapons/" ) != std::string::npos && mdl.find( "weapons/v_" ) == std::string::npos &&
	     ( should_draw_attachment_vis || should_draw_attachment_invis ) ) {
		if ( should_draw_attachment_invis ) {
