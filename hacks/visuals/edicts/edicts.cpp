...
void n_edicts::impl_t::sun_set_mode( )
{
	...
	if ( !GET_VARIABLE( g_variables.m_sun_set, bool ) ) {
		cl_csm_shadows->set_value( GET_VARIABLE( g_variables.m_shadows, bool ) ? 1 : 0 );
		return;
	}
...
void n_edicts::impl_t::skybox( int stage )
{
	...
	constexpr auto custom_skybox_index = static_cast< int >( skyboxList.size( ) - 1 );
	const int skybox = GET_VARIABLE( g_variables.m_sky_box, int );

	if ( stage == e_client_frame_stage::render_start && skybox > 0 && skybox < custom_skybox_index ) {
		if ( g_interfaces.m_convar->find_var( "r_3dsky" ) )
			g_interfaces.m_convar->find_var( "r_3dsky" )->set_value( 0 );

		memesclass->loadSky( skyboxList[ skybox ] );
	} else if ( skybox == custom_skybox_index && stage == e_client_frame_stage::render_start ) {
