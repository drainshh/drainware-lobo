...
{
	if ( GET_VARIABLE( g_variables.m_viewmodel_fov, bool ) )
		setup.m_fov_viewmodel = static_cast< float >( GET_VARIABLE( g_variables.m_viewmodel_fov_value, int ) );

	if ( GET_VARIABLE( g_variables.m_dof, bool ) ) {
		const float focus = std::clamp( GET_VARIABLE( g_variables.m_dof_focus_distance, float ), 16.f, 4096.f );
		const float near_depth = std::clamp( GET_VARIABLE( g_variables.m_dof_near_depth, float ), 1.f, 2048.f );
		const float far_depth = std::clamp( GET_VARIABLE( g_variables.m_dof_far_depth, float ), 1.f, 4096.f );
		const float blur = std::clamp( GET_VARIABLE( g_variables.m_dof_blur_amount, float ), 0.f, 12.f );
		setup.m_near_blur_depth = std::max( 1.f, focus - near_depth );
		setup.m_near_focus_depth = focus;
		setup.m_far_focus_depth = focus + near_depth;
		setup.m_far_blur_depth = focus + far_depth;
		setup.m_near_blur_radius = blur;
		setup.m_far_blur_radius = blur;
		setup.m_dof_quality = GET_VARIABLE( g_variables.m_dof_smooth, bool ) ? 2 : 1;
	}
