...
bool n_interfaces::impl_t::on_attach( )
{
	...
	m_network_string_table_container =
		static_cast< c_network_string_table_container* >( g_modules[ ENGINE_DLL ].find_interface( "VEngineClientStringTable001" ) );
	if ( m_network_string_table_container )
		g_console.print( std::format( "found INetworkStringTableContainer @ {:p}",
		                               reinterpret_cast< void* >( m_network_string_table_container ) )
			                 .c_str( ) );
	else
		g_console.print< n_console::log_level::WARNING >( "failed to find string table container; named particles may not precache" );
	...
	m_effects = static_cast< c_effects* >( g_modules[ CLIENT_DLL ].find_interface( "IEffects001" ) );
	...
	const auto hud_chat_offset = g_modules[ CLIENT_DLL ].find_pattern( "E8 ? ? ? ? 8B 4F ? 85 C9 74 06 51" );
	if ( hud_chat_offset )
		m_hud_chat = *reinterpret_cast< c_hud_chat** >( reinterpret_cast< std::uintptr_t >( m_client_mode ) +
		                                                *reinterpret_cast< std::uint8_t* >( hud_chat_offset + 7 ) );
	if ( m_hud_chat )
		g_console.print( std::format( "found CHudChat @ {:p}", reinterpret_cast< void* >( m_hud_chat ) ).c_str( ) );
	else
		g_console.print< n_console::log_level::WARNING >( "failed to find CHudChat, chat prints will fall back to console" );
